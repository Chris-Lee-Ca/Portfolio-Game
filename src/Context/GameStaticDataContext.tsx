...
export const GameStaticDataContextProvider = ({ children }: any) => {
    ...
    if (value.queryData.loading) console.log("fetching sanity data");
    if (value.queryData.error) console.error("error: ", error);

    return (
        <>
            <GameStaticDataContext.Provider value={value}>{children}</GameStaticDataContext.Provider>
        </>
    );
};
