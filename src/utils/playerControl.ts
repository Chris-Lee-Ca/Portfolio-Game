...
export const handleAllowedMovingDirestion = (
    playerState: PlayerStateInterface,
    setPlayerState: Dispatch<SetStateAction<PlayerStateInterface>>,
    allowdedCommand: commandType[],
    htmlCommand: htmlCommandType,
) => {
    const command = commandMapping[htmlCommand] as commandType;
    if (allowdedCommand.includes(command)) return handleOnClickArrowButton(playerState, setPlayerState, command);
};

export const handleOnClickArrowButton = (
    playerState: PlayerStateInterface,
    setPlayerState: Dispatch<SetStateAction<PlayerStateInterface>>,
    command: commandType,
) => {
...
export const triggerInteractionEvent = (
    ...
    if (!currentMapBox.canIntactive) return;
...
export const triggerForceTriggerEvent = (
    playerPosition: [number, number],
    mapDesign: mapBoxDesignInterface[][],
    ...
    if (!currentMapBox.forceTriggerFunction) return;
