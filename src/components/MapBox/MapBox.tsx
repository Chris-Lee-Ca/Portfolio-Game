...
const MapBox = (props: MapBoxPropsInterface) => {
    ...
    const handleAddMapEdge = (): CSSProperties => {
        ...
        if (col === 0) borderStyle["borderLeft"] = border;
        if (row === 0) borderStyle["borderTop"] = border;
        if (col === mapDesign[row].length - 1) borderStyle["borderRight"] = border;
        return borderStyle;
    };
    ...
    const isAddCliff = () => {
        const row = location[0];
        if (row === mapDesign.length - 1) return true;
        return false;
    };
    ...
    return (
        <>
            <Container style={handleAddMapEdge()}>
                ...
                {isAddCliff() && (
                    <CliffImage src={require("../../assets/game_images/cliff.png")} style={handleAddCliffEdge()} />
                )}
                {children}
            </Container>
        </>
