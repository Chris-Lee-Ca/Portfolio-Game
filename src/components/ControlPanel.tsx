...
const ControlPanel = (props: ControlPanelPropsInterface) => {
    ...
    const handleCloseWindow = (key: string) => {
        if (!htmlInteractType.includes(key)) return;
        if (isOpenPopUpWindow) setGameInfoState({ ...gameInfoState, isOpenPopUpWindow: false });
        if (isOpenDialogWindow) setDialogState({ ...dialogState, isOpenDialogWindow: false });
    };
    const keyBoardListener = useCallback(
        (e: any) => {
            const htmlCommandType = [...htmlMovingType, ...htmlInteractType];
            if (!htmlCommandType.includes(e.key)) return;
            if (isOpenPopUpWindow || isOpenDialogWindow) return handleCloseWindow(e.key);
            if (htmlMovingType.includes(e.key))
                handleAllowedMovingDirestion(playerState, setPlayerState, allowedMovingDirestion, e.key);
            if (htmlInteractType.includes(e.key)) handleInteractionButton();
        },
                ...
                <DirectionKeyContainer item xs={6}>
                    <Box component="div">
                        <ControlButton
                            ...
                            type="direction"
                            correspondingKeys={["ArrowUp"]}
                            onClick={() => handleOnClickArrowButton("ArrowUp")}
                        >
                            <ArrowDropUpIcon />
                        </ControlButton>
                    </Box>
                    <Box component="div" display={"flex"}>
                        <ControlButton
                            type="direction"
                            correspondingKeys={["ArrowLeft"]}
                            onClick={() => handleOnClickArrowButton("ArrowLeft")}
                        >
                            <ArrowLeftIcon />
                        </ControlButton>
                        <ControlButton
                            type="direction"
                            correspondingKeys={["ArrowDown"]}
                            onClick={() => handleOnClickArrowButton("ArrowDown")}
                        >
                            <ArrowDropDownIcon />
                        </ControlButton>
                        <ControlButton
                            type="direction"
                            correspondingKeys={["ArrowRight"]}
                            onClick={() => handleOnClickArrowButton("ArrowRight")}
                        >
                            <ArrowRightIcon />
                        </ControlButton>
                    </Box>
